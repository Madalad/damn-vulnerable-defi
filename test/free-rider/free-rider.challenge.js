...
describe("[Challenge] Free Rider", function () {
    ...
    it("Exploit", async function () {
        ...
        /*
        `buyMany()` uses same msg.value for each purchase
        => can buy arbitrarily many NFTs for the price of one

        Moreover:
            - `buyOne()` function has another critical error
            - transfers nft to buyer BEFORE transferring ether to seller
            - it uses `token.ownerOf(tokenID)` to determine seller
            - this will always point to the BUYER instead
            - as such, attacker also drains 6 * 15 eth from the marketplace
        */
